...
static int mtm_test (HANDLE *, char *, const int);
static int mtm_load (struct module_data *, HANDLE *, const int);
...
static int mtm_test(HANDLE *f, char *t, const int start)
{
    uint8 buf[4];
...
static int mtm_load(struct module_data *m, HANDLE *f, const int start)
{
    ...
    hread(&mfh.magic, 3, 1, f);		/* "MTM" */
    mfh.version = hread_8(f);		/* MSN=major, LSN=minor */
    hread(&mfh.name, 20, 1, f);		/* ASCIIZ Module name */
    mfh.tracks = hread_16l(f);		/* Number of tracks saved */
    mfh.patterns = hread_8(f);		/* Number of patterns saved */
    mfh.modlen = hread_8(f);		/* Module length */
    mfh.extralen = hread_16l(f);		/* Length of the comment field */
    mfh.samples = hread_8(f);		/* Number of samples */
    mfh.attr = hread_8(f);		/* Always zero */
    mfh.rows = hread_8(f);		/* Number rows per track */
    mfh.channels = hread_8(f);		/* Number of tracks per pattern */
    hread(&mfh.pan, 32, 1, f);		/* Pan positions for each channel */
	...
	hread(&mih.name, 22, 1, f);		/* Instrument name */
	mih.length = hread_32l(f);		/* Instrument length in bytes */
	mih.loop_start = hread_32l(f);		/* Sample loop start */
	mih.loopend = hread_32l(f);		/* Sample loop end */
	mih.finetune = hread_8(f);		/* Finetune */
	mih.volume = hread_8(f);			/* Playback volume */
	mih.attr = hread_8(f);			/* &0x01: 16bit sample */
