...
{
	devDef262_MAME.Update(param, 0, NULL);
}
...
static UINT8 device_start_ymf262_mame(const DEV_GEN_CFG* cfg, DEV_INFO* retDevInf)
{
	...
	ymf262_set_update_handler(chip, stream_update3812_mame, chip);
...
{
	devDef262_AdLibEmu.Update(param, 0, NULL);
}
...
static UINT8 device_start_ymf262_adlibemu(const DEV_GEN_CFG* cfg, DEV_INFO* retDevInf)
{
	...
	if (chip == NULL)
		return 0xFF;
	adlib_OPL3_set_update_handler(chip, stream_update3812_adlibemu, chip);
