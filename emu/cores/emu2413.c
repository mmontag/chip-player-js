/***********************************************************************************
  ...
  Further modifications:
    - smooth per-channel panning by Maxim
    - 6-channel VRC7 mode by Valley Bell
    - fixed drums being turned off by frequency writes to 0x26-0x28 -Valley Bell
    - reduced instrument table size from 16 to 8 bytes per instrument -Valley Bell
    - ported linear resampling from old EMU2413 -Valley Bell
  original Git repository: https://github.com/digital-sound-antiques/emu2413
...
#define RATE_ADJUST(opll,x) (opll->native?x:(uint32_t)((double)(x)*opll->clk/72/opll->rate + 0.5))        /* added 0.5 to round the value*/
...
/* Basic voice Data */
static EOPLL_PATCH default_patch[OPLL_TONE_NUM][(16 + 3) * 2];
...
makeSinTable (void)
{
  ...
  {
    fullsintable[i] = (uint32_t) lin2db (sin (2.0 * M_PI * i / PG_WIDTH) );
  }
...
makePmTable (void)
{
  ...
  for (i = 0; i < PM_PG_WIDTH; i++)
    pmtable[i] = (int32_t) ((double) PM_AMP * pow (2, (double) PM_DEPTH * saw (2.0 * M_PI * i / PM_PG_WIDTH) / 1200));
}
...
makeAmTable (void)
{
  ...
  for (i = 0; i < AM_PG_WIDTH; i++)
    amtable[i] = (int32_t) ((double) AM_DEPTH / 2 / DB_STEP * (1.0 + saw (2.0 * M_PI * i / PM_PG_WIDTH)));
}

/* Phase increment counter table */
static void
makeDphaseTable (EOPLL * opll)
{
  ...
  for (fnum = 0; fnum < 512; fnum++)
    for (block = 0; block < 8; block++)
      for (ML = 0; ML < 16; ML++)
        opll->dphaseTable[fnum][block][ML] = RATE_ADJUST (opll, ((fnum * mltable[ML]) << block) >> (20 - DP_BITS));
}
  ...
  for (AR = 0; AR < 16; AR++)
    for (Rks = 0; Rks < 16; Rks++)
    {
      ...
      case 15:
        ...
        break;
      default:
        opll->dphaseARTable[AR][Rks] = RATE_ADJUST (opll, (3 * (RL + 4) << (RM + 1)));
        break;
      }
    }
}
  ...
  for (DR = 0; DR < 16; DR++)
    for (Rks = 0; Rks < 16; Rks++)
    {
      ...
      case 0:
        ...
        break;
      default:
        opll->dphaseDRTable[DR][Rks] = RATE_ADJUST (opll, (RL + 4) << (RM - 1));
        break;
      }
    }
}
...
void
EOPLL_dump2patch (const uint8_t * dump, EOPLL_PATCH * patch)
{
...
void
EOPLL_getDefaultPatch (int32_t type, int32_t num, EOPLL_PATCH * patch)
{
  EOPLL_dump2patch (default_inst[type] + num * 8, patch);
}
...
void
EOPLL_setPatch (EOPLL * opll, const uint8_t * dump)
{
  EOPLL_PATCH patch[2];
  int i;
  ...
  for (i = 0; i < 19; i++)
  {
    EOPLL_dump2patch (dump + i * 8, patch);
...
void
EOPLL_patch2dump (const EOPLL_PATCH * patch, uint8_t * dump)
{
...
INLINE uint32_t
calc_eg_dphase (EOPLL * opll, EOPLL_SLOT * slot)
{
  ...
  case SETTLE:
    return opll->dphaseDRTable[15][0];
...
#define UPDATE_PG(opll,S) (S)->dphase = opll->dphaseTable[(S)->fnum][(S)->block][(S)->patch->ML]
#define UPDATE_TLL(S)\
(((S)->type==0)?\
...
#define UPDATE_EG(opll,S) (S)->eg_dphase = calc_eg_dphase(opll,S)
#define UPDATE_ALL(opll,S)\
  UPDATE_PG(opll,S);\
  UPDATE_TLL(S);\
  UPDATE_RKS(S);\
  UPDATE_WF(S); \
  UPDATE_EG(opll,S)             /* EG should be updated last. */


/* Slot key on  */
INLINE void
slotOn (EOPLL * opll, EOPLL_SLOT * slot)
{
  slot->eg_mode = ATTACK;
  slot->eg_phase = 0;
  slot->phase = 0;
...
INLINE void
slotOn2 (EOPLL * opll, EOPLL_SLOT * slot)
{
  slot->eg_mode = ATTACK;
  slot->eg_phase = 0;
  UPDATE_EG(opll, slot);
...
/* Slot key off */
INLINE void
slotOff (EOPLL * opll, EOPLL_SLOT * slot)
{
...
/* Channel key on */
INLINE void
keyOn (EOPLL * opll, int32_t i)
{
  if (!opll->slot_on_flag[i * 2])
    slotOn (opll, MOD(opll,i));
  if (!opll->slot_on_flag[i * 2 + 1])
    slotOn (opll, CAR(opll,i));
  opll->key_status[i] = 1;
}

/* Channel key off */
INLINE void
keyOff (EOPLL * opll, int32_t i)
{
  if (opll->slot_on_flag[i * 2 + 1])
...
keyOn_SD (EOPLL * opll)
{
  if (!opll->slot_on_flag[SLOT_SD])
    slotOn (opll, CAR(opll,7));
}

INLINE void
keyOn_TOM (EOPLL * opll)
{
  if (!opll->slot_on_flag[SLOT_TOM])
    slotOn (opll, MOD(opll,8));
}
...
keyOn_HH (EOPLL * opll)
{
  if (!opll->slot_on_flag[SLOT_HH])
    slotOn2 (opll, MOD(opll,7));
}

INLINE void
keyOn_CYM (EOPLL * opll)
{
  if (!opll->slot_on_flag[SLOT_CYM])
    slotOn2 (opll, CAR(opll,8));
}

/* Drum key off */
INLINE void
keyOff_BD (EOPLL * opll)
{
  keyOff (opll, 6);
}

INLINE void
keyOff_SD (EOPLL * opll)
{
  if (opll->slot_on_flag[SLOT_SD])
    slotOff (opll, CAR(opll,7));
}

INLINE void
keyOff_TOM (EOPLL * opll)
{
  if (opll->slot_on_flag[SLOT_TOM])
    slotOff (opll, MOD(opll,8));
}

INLINE void
keyOff_HH (EOPLL * opll)
{
  if (opll->slot_on_flag[SLOT_HH])
    slotOff (opll, MOD(opll,7));
}

INLINE void
keyOff_CYM (EOPLL * opll)
{
  if (opll->slot_on_flag[SLOT_CYM])
    slotOff (opll, CAR(opll,8));
}

/* Change a voice */
INLINE void
setPatch (EOPLL * opll, int32_t i, uint8_t num)
{
...
INLINE void
setSlotPatch (EOPLL_SLOT * slot, EOPLL_PATCH * patch)
{
  slot->patch = patch;
}
...
INLINE void
setSustine (EOPLL * opll, uint8_t c, uint8_t sustine)
{
...
INLINE void
setVolume (EOPLL * opll, uint8_t c, uint8_t volume)
{
  CAR(opll,c)->volume = volume;
}

INLINE void
setSlotVolume (EOPLL_SLOT * slot, uint8_t volume)
{
  slot->volume = volume;
}
...
INLINE void
setFnumber (EOPLL * opll, uint8_t c, uint16_t fnum)
{
...
INLINE void
setBlock (EOPLL * opll, uint8_t c, uint8_t block)
{
...
void
EOPLL_copyPatch (EOPLL * opll, int32_t num, EOPLL_PATCH * patch)
{
...
static void
EOPLL_SLOT_reset (EOPLL_SLOT * slot, uint8_t type)
{
  slot->type = type;
...
static void
internal_refresh (EOPLL * opll)
{
  opll->native = (fabs((opll->clk / 72.0f) - opll->rate) < 2.0f);
  makeDphaseTable (opll);
  makeDphaseARTable (opll);
  makeDphaseDRTable (opll);
  opll->pm_dphase = (uint32_t) RATE_ADJUST (opll, PM_SPEED * PM_DP_WIDTH / (opll->clk / 72.0));
  opll->am_dphase = (uint32_t) RATE_ADJUST (opll, AM_SPEED * AM_DP_WIDTH / (opll->clk / 72.0));
}

static void
maketables (EOPLL * opll, uint32_t c, uint32_t r)
{
  if (tablesInit == 0)
  {
    tablesInit = 1;
...
EOPLL *
EOPLL_new (uint32_t clk, uint32_t rate)
{
  EOPLL *opll;
  int32_t i;
...
void
EOPLL_reset_patch (EOPLL * opll, int32_t type)
{
  int32_t i;

  for (i = 0; i < 19 * 2; i++)
    EOPLL_copyPatch (opll, i, &default_patch[type % OPLL_TONE_NUM][i]);
}

/* Reset whole of OPLL except patch datas. */
void
EOPLL_reset (EOPLL * opll)
{
  int32_t i;
  ...
  for (i = 0; i <18; i++)
    EOPLL_SLOT_reset(&opll->slot[i], i%2);
  ...
  opll->realstep = (uint32_t) ((1U << 31) / opll->rate);
  opll->opllstep = (uint32_t) ((1U << 31) / (opll->clk / 72.0));
  opll->oplltime = 0;
  //for (i = 0; i < 15; i++)
  //  opll->pan[i] = 0.0f;
...
EOPLL_forceRefresh (EOPLL * opll)
{
  ...
  for (i = 0; i < 18; i++)
  {
    UPDATE_PG (opll, &opll->slot[i]);
    UPDATE_RKS (&opll->slot[i]);
    UPDATE_TLL (&opll->slot[i]);
    UPDATE_WF (&opll->slot[i]);
    UPDATE_EG (opll, &opll->slot[i]);
  }
}

void
EOPLL_set_rate (EOPLL * opll, uint32_t r)
{
  if (opll->quality)
    opll->rate = (opll->clk + 36) / 72; //49716
  else
    opll->rate = r;
  internal_refresh (opll);
  opll->rate = r;
...
void
EOPLL_set_quality (EOPLL * opll, uint8_t q)
{
  opll->quality = q;
  EOPLL_set_rate (opll, opll->rate);
}
...
/* PG */
INLINE void
calc_phase (EOPLL_SLOT * slot, int32_t lfo)
{
  if (slot->patch->PM)
...
/* EG */
static void
calc_envelope (EOPLL * opll, EOPLL_SLOT * slot, int32_t lfo)
{
...
/* CARRIOR */
INLINE int16_t
calc_slot_car (EOPLL_SLOT * slot, int32_t fm)
{
...
/* SNARE */
INLINE int16_t
calc_slot_snare (EOPLL_SLOT * slot, uint32_t noise)
{
  if(slot->egout>=(DB_MUTE-1))
    return 0;
 ...
 */
INLINE int16_t
calc_slot_cym (EOPLL_SLOT * slot, uint32_t pgout_hh)
{
  uint32_t dbout;
...
*/
INLINE int16_t
calc_slot_hat (EOPLL_SLOT *slot, int32_t pgout_cym, uint32_t noise)
{
  uint32_t dbout;
...
static void
update_output (EOPLL * opll)
{
#define INST_VOL_MULT 8
#define RHYTHM_VOL_MULT 16
  ...
  memset(opll->ch_out, 0, 15 * sizeof(int16_t));
  /* CH1-6 */
  for (i = 0; i < 6; i++)
    if (!(opll->mask & EOPLL_MASK_CH (i)) && (CAR(opll,i)->eg_mode != FINISH))
 ...
 if (! opll->vrc7_mode)
 {
  ...
  {
    if (!(opll->mask & EOPLL_MASK_CH (6)) && (CAR(opll,6)->eg_mode != FINISH))
      opll->ch_out[6] = calc_slot_car (CAR(opll,6), calc_slot_mod(MOD(opll,6))) * INST_VOL_MULT;
  }
  else
  {
    if (!(opll->mask & EOPLL_MASK_BD) && (CAR(opll,6)->eg_mode != FINISH))
  ...
  {
    if (!(opll->mask & EOPLL_MASK_CH(8)) && (CAR(opll,8)->eg_mode != FINISH))
      opll->ch_out[8] = calc_slot_car (CAR(opll,8), calc_slot_mod (MOD(opll,8))) * INST_VOL_MULT;
  }
  else
  {
    if (!(opll->mask & EOPLL_MASK_TOM) && (MOD(opll,8)->eg_mode != FINISH))
      opll->ch_out[12] = calc_slot_tom (MOD(opll,8)) * RHYTHM_VOL_MULT;
    if (!(opll->mask & EOPLL_MASK_CYM) && (CAR(opll,8)->eg_mode != FINISH))
...
INLINE void
mix_output_stereo(EOPLL *opll, int32_t out[2]) {
  int ch;
  out[0] = out[1] = 0;
  for(ch=0;ch<15;ch++) {
    add_channel_lr(opll->ch_out[ch], opll->pan[ch][0], opll->pan[ch][1], out);
  }
}

void
EOPLL_calc_stereo (EOPLL * opll, UINT32 samples, DEV_SMPL **out)
{
  ...
  if (!opll->quality)
  {
    ...
    {
      update_output(opll);
      mix_output_stereo(opll, buffers);
      bufMO[i] = buffers[0];
      bufRO[i] = buffers[1];
    }
  }
  else
  {
    for( i=0; i < samples ; i++ )
    {
      while (opll->realstep > opll->oplltime)
      {
        ...
        update_output(opll);
        mix_output_stereo(opll, opll->snext);
      }
      opll->oplltime -= opll->realstep;
      bufMO[i] = (DEV_SMPL) (((double) opll->snext[0] * (opll->opllstep - opll->oplltime)
                            + (double) opll->sprev[0] * opll->oplltime) / opll->opllstep);
      bufRO[i] = (DEV_SMPL) (((double) opll->snext[1] * (opll->opllstep - opll->oplltime)
...
uint32_t
EOPLL_setMask (EOPLL * opll, uint32_t mask)
{
  uint32_t ret;
...
uint32_t
EOPLL_toggleMask (EOPLL * opll, uint32_t mask)
{
  uint32_t ret;
...
void EOPLL_SetMuteMask(EOPLL* opll, UINT32 MuteMask)
{
  ...
  opll->mask = 0;
  for (CurChn = 0; CurChn < 15; CurChn ++)
  {
    switch(CurChn)
    {
    case 9:
      ChnMsk = EOPLL_MASK_BD;
      break;
    case 10:
      ChnMsk = EOPLL_MASK_SD;
      break;
    case 11:
      ChnMsk = EOPLL_MASK_TOM;
      break;
    case 12:
      ChnMsk = EOPLL_MASK_CYM;
      break;
    case 13:
      ChnMsk = EOPLL_MASK_HH;
      break;
    default:
      ChnMsk = EOPLL_MASK_CH(CurChn);
      break;
    }
...
void
EOPLL_writeReg (EOPLL * opll, UINT8 reg, UINT8 data)
{
  uint8_t i, v, ch;
  ...
  case 0x00:
    ...
    for (i = 0; i < 9; i++)
    {
      ...
      {
        UPDATE_PG (opll, MOD(opll,i));
        UPDATE_RKS (MOD(opll,i));
        UPDATE_EG (opll, MOD(opll,i));
      }
    }
    break;
  ...
  case 0x01:
    ...
    for (i = 0; i < 9; i++)
    {
      if (opll->patch_number[i] == 0)
      {
        UPDATE_PG (opll, CAR(opll,i));
        UPDATE_RKS (CAR(opll,i));
        UPDATE_EG (opll, CAR(opll,i));
      }
    }
    break;
  ...
  case 0x0e:
    ...
    UPDATE_ALL (opll, MOD(opll,6));
    UPDATE_ALL (opll, CAR(opll,6));
    UPDATE_ALL (opll, MOD(opll,7));
    UPDATE_ALL (opll, CAR(opll,7));
    UPDATE_ALL (opll, MOD(opll,8));
    UPDATE_ALL (opll, CAR(opll,8));
  ...
  case 0x18:
    ...
    UPDATE_ALL (opll, MOD(opll,ch));
    UPDATE_ALL (opll, CAR(opll,ch));
    break;
  ...
  case 0x28:
    ...
    if (ch < 0x06 || ! (opll->reg[0x0e] & 32))
    {
      // fix to prevent commands 0x26-0x28 from turning drums (BD, SD, CYM) off -VB
      if (data & 0x10)
        keyOn (opll, ch);
      else
        keyOff (opll, ch);
  ...
  case 0x38:
    ...
    UPDATE_ALL (opll, MOD(opll,reg - 0x30));
    UPDATE_ALL (opll, CAR(opll,reg - 0x30));
    break;
...
void
EOPLL_writeIO (EOPLL * opll, UINT8 adr, UINT8 val)
{
  if (adr & 1)
...
/* STEREO MODE (OPT) */
void
EOPLL_set_pan (EOPLL * opll, uint32_t ch, int16_t pan)
{
  if (ch >= 15)
    return;
