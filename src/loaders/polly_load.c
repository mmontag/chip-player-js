...
static int polly_test(HANDLE *, char *, const int);
static int polly_load(struct module_data *, HANDLE *, const int);
...
static void decode_rle(uint8 *out, HANDLE *f, int size)
{
	int i;
...
static int polly_test(HANDLE *f, char *t, const int start)
{
	int i;
	uint8 *buf;
...
static int polly_load(struct module_data *m, HANDLE *f, const int start)
{
