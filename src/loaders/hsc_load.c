...
static int hsc_test (HANDLE *, char *, const int);
static int hsc_load (struct module_data *, HANDLE *, const int);
...
static int hsc_test(HANDLE *f, char *t, const int start)
{
    int p, i, r, c;
    uint8 buf[1200];

    hseek(f, 128 * 12, SEEK_CUR);

    if (hread(buf, 1, 51, f) != 51)
	return -1;
...
static int hsc_load(struct module_data *m, HANDLE *f, const int start)
{
    ...
    /* Read orders */
    for (pat = i = 0; i < 51; i++) {
	hread (&mod->xxo[i], 1, 1, f);
	if (mod->xxo[i] & 0x80)
	    break;			/* FIXME: jump line */
	if (mod->xxo[i] > pat)
	    pat = mod->xxo[i];
    }
    hseek(f, 50 - i, SEEK_CUR);
    mod->len = i;
    mod->pat = pat + 1;
