...
static int umx_test (HANDLE *, char *, const int);
static int umx_load (struct module_data *, HANDLE *, const int);
...
static int umx_test(HANDLE *f, char *t, const int start)
{
	int i, offset = -1;
	uint8 buf[TEST_SIZE], *b = buf;
	uint32 id;

	if (hread(buf, 1, TEST_SIZE, f) < TEST_SIZE)
		return -1;
;
	id = readmem32b(b);
...
static int umx_load(struct module_data *m, HANDLE *f, const int start)
{
	int i;
	uint8 buf[TEST_SIZE], *b = buf;
	uint32 id;
