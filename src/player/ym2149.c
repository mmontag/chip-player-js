...
/*
 * Modified for xmp by Claudio Matsuoka, 24-Aug-2011
 */
...
static int32 mixbuf[MAX_SPL_PER_PASS], * mixptr;
...
static void add_entry(entrylist_t *l, uint32 reg, uint32 v, uint32 cycle)
{
  ...
  if (1) {
    ...
    */
    ...
    for (p=mixerR.e; p<mixerR.c; ) {
      const uint32 current_cycle = p->cycle;
      int newcenter = 0;
...
*               return # mixed sample                 *
******************************************************/
unsigned int YM_mix(uint32 cycle2mix)
{
...
*************************************/
void YM_writereg(uint8 reg, uint8 v, uint32 cycle)
{
  switch(reg)
  {
    case YM_PERL(0): case YM_PERH(0):
...
********************************************/
uint8 YM_readreg(uint8 reg, uint32 cycle)
{
    /*return ym.data[reg];*/
...
********************************************/
void YM_subcycle(uint32 subcycle)
{
...
*                  Yamaha get buffer                  *
******************************************************/
uint32 *YM_get_buffer(void)
{
  return (uint32 *)mixbuf;
}
