...
next_order:
	...
	num_rows = m->xxp[m->xxo[ord]]->rows;
	if (f->jumpline >= num_rows)
	    f->jumpline = 0;
	f->row = f->jumpline;
	f->jumpline = 0;
	...
	for (; f->row < num_rows; f->row++) {
	    ...
	    /* Frame processing */
	    for (frame = 0; frame < (p->tempo * (1 + f->delay)); frame++) {
		...
		xmp_drv_echoback(ctx, (frame << 4) | XMP_ECHO_FRM);

		play_frame(ctx, frame);
