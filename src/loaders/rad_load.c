...
static int rad_test(HANDLE *, char *, const int);
static int rad_load(struct module_data *, HANDLE *, const int);
...
static int rad_test(HANDLE *f, char *t, const int start)
{
	char buf[16];

	if (hread(buf, 1, 16, f) < 16)
		return -1;
...
static int rad_load(struct module_data *m, HANDLE *f, const int start)
{
	...
	hseek(f, 16, SEEK_SET);		/* skip magic */
	version = hread_8(f);
	flags = hread_8(f);
	...
	for (i = 0; i < mod->pat; i++) {
		...
		hseek(f, start + ppat[i], SEEK_SET);

		do {
			r = hread_8(f);		/* Row number */
