...
#warning FIXME: med tempos are incorrectly handled
	...
	if (V (1)) {
	    report ("\n[%2x] %-40.40s ", i,
		expdata && expdata->iinfo ? mmd + (uint32)expdata->iinfo +
		expdata->i_ext_entrsz * i : "");
	    report ("%s ", instr->type + 2 <= NUM_INST_TYPES ?
		inst_type[instr->type + 2] : "???");
	}

	if (expdata)
	    instrext = (struct InstrExt *)(mmd + (uint32)expdata->exp_smp);
	...
	case -2:			/* Hybrid */
	    ...
	    xxs[smp_idx].lpe = xxs[smp_idx].lps + 2 * song->sample[i].replen;
	    xxs[smp_idx].flg = song->sample[i].replen > 1 ? WAVE_LOOPING : 0;

	    if (V (1)) {
		report ("%05x %05x %05x %02x %02x %+1d ",
		    xxs[smp_idx].len, xxs[smp_idx].lps, xxs[smp_idx].lpe,
		    xxi[i][0].vol, (uint8)xxi[i][0].xpo, xxi[i][0].fin >> 4);
	    }

	    xmp_drv_loadpatch (NULL, smp_idx, xmp_ctl->c4rate,
		XMP_SMP_NOLOAD, &xxs[smp_idx], (char *)instr + sizeof
		(struct InstrHdr));
	...
	case -1:			/* Synthetic */
	    ...
	    if (V (1))
		report ("VS:%02x WS:%02x WF:%02x %02x %02x %+1d ",
		    synth->volspeed, synth->wfspeed, synth->wforms,
		    song->sample[i].svol, (uint8) song->sample[i].strans,
		    expdata ? instrext->finetune : 0);

	    xxi[i] = calloc (sizeof (struct xxm_instrument), synth->wforms);
		...
		xxi[i][j].sid = smp_idx;
		xxi[i][j].fin = expdata ? instrext->finetune << 4 : 0;
		...
		xmp_drv_loadpatch (NULL, smp_idx, xmp_ctl->c4rate,
		    XMP_SMP_NOLOAD | XMP_SMP_8X, &xxs[smp_idx],
		    &synthwf->wfdata[0]);
