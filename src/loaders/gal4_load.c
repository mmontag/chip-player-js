...
static int gal4_test(HANDLE *, char *, const int);
static int gal4_load(struct module_data *, HANDLE *, const int);
...
static int gal4_test(HANDLE *f, char *t, const int start)
{
        if (hread_32b(f) != MAGIC4('R', 'I', 'F', 'F'))
		return -1;

	hread_32b(f);

	if (hread_32b(f) != MAGIC4('A', 'M', 'F', 'F'))
		return -1;

	if (hread_32b(f) != MAGIC4('M', 'A', 'I', 'N'))
		return -1;
...
static void get_main(struct module_data *m, int size, HANDLE *f, void *parm)
{
	...
	mod->spd = hread_8(f);
	mod->bpm = hread_8(f);
	hread_16l(f);		/* unknown - 0x01c5 */
	hread_16l(f);		/* unknown - 0xff00 */
	hread_8(f);		/* unknown - 0x80 */
}

static void get_ordr(struct module_data *m, int size, HANDLE *f, void *parm)
{
	struct xmp_module *mod = &m->mod;
	int i;
...
static void get_patt_cnt(struct module_data *m, int size, HANDLE *f, void *parm)
{
	struct xmp_module *mod = &m->mod;
	int i;

	i = hread_8(f) + 1;		/* pattern number */

	if (i > mod->pat)
		mod->pat = i;
}

static void get_inst_cnt(struct module_data *m, int size, HANDLE *f, void *parm)
{
	struct xmp_module *mod = &m->mod;
	int i;

	hread_8(f);			/* 00 */
	i = hread_8(f) + 1;		/* instrument number */
	
	if (i > mod->ins)
		mod->ins = i;

	hseek(f, 28, SEEK_CUR);		/* skip name */
...
static void get_inst(struct module_data *m, int size, HANDLE *f, void *parm)
{
	...
	hread_8(f);		/* 00 */
	i = hread_8(f);		/* instrument number */

	hread(&mod->xxi[i].name, 1, 28, f);
	str_adj((char *)mod->xxi[i].name);

	mod->xxi[i].nsm = hread_8(f);

	for (j = 0; j < 108; j++) {
		mod->xxi[i].map[j].ins = hread_8(f);
	}

	hseek(f, 11, SEEK_CUR);		/* unknown */
	vwf = hread_8(f);			/* vibrato waveform */
	vsw = hread_8(f);			/* vibrato sweep */
	hread_8(f);			/* unknown */
	hread_8(f);			/* unknown */
	vde = hread_8(f) / 4;		/* vibrato depth */
	vra = hread_16l(f) / 16;		/* vibrato speed */
	hread_8(f);			/* unknown */

	val = hread_8(f);			/* PV envelopes flags */
	if (LSN(val) & 0x01)
	...
	for (j = 0; j < mod->xxi[i].nsm; j++, data->snum++) {
		...
		hread(&mod->xxs[data->snum].name, 1, 28, f);
		str_adj((char *)mod->xxs[data->snum].name);
	
		mod->xxi[i].sub[j].pan = hread_8(f) * 4;
		...
		mod->xxs[data->snum].len = hread_32l(f);
		mod->xxs[data->snum].lps = hread_32l(f);
		mod->xxs[data->snum].lpe = hread_32l(f);
...
static int gal4_load(struct module_data *m, HANDLE *f, const int start)
{
	struct xmp_module *mod = &m->mod;
	iff_handle handle;
	int i, offset;
