...
typedef struct
{
    ...
    */
	...
	void (*irqhandler)(void *param, int irq);		/* IRQ function handler */
...
// TODO: get rid of those variables (move into YM2151 struct)
	...
	case 0x00:
		...
		case 0x14:	/* CSM, irq flag reset, irq enable, timer start/stop */
			...
			if (v&0x10)	/* reset timer A irq flag */
			{
				...
				chip->status &= ~1;
				if ((oldstate==1) && (chip->irqhandler)) chip->irqhandler(chip->chipInf, 0);
			}
			...
			if (v&0x20)	/* reset timer B irq flag */
			{
				int oldstate = chip->status & 3;
				chip->status &= ~2;
				if ((oldstate==2) && (chip->irqhandler)) chip->irqhandler(chip->chipInf, 0);
			}
...
void ym2151_set_irq_handler(void *chip, void(*handler)(void *param, int irq))
{
	YM2151 *PSG = (YM2151 *)chip;
