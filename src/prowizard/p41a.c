/*
 * The_Player_4.1a.c   Copyright (C) 1997 Asle / ReDoX
 ...
 *
 * $Id: p41a.c,v 1.2 2007-09-29 17:01:33 cmatsuoka Exp $
 */
...
static int test_p41a (uint8 *, int);
static int depack_p41a (FILE *, FILE *);

struct pw_format pw_p41a = {
        "P41A",
        "The Player 4.1A",
        0x00,
        test_p41a,
...
static int depack_p41a (FILE *in, FILE *out)
{
	...
	read32b(in);			/* bypass check ID */
	PatMax = read8(in);		/* read Real number of pattern */
	...
	for (i = 0; i < nsmp; i++) {
		ins.addr = read32b(in);			/* read sample addr */
		SampleAddress[i] = ins.addr;	
		ins.size = read16b(in);			/* read sample size */
		SampleSize[i] = ins.size * 2;		/* read sample size */
		ssize += SampleSize[i];
	...
	for (c1 = 0; c1 < len; c1++)	/* create pattern list */
		write8(out, c1);
	for (; c1 < 128; c1++)
		write8(out, 0);
...
static int test_p41a(uint8 *data, int s)
{
	int start, ssize;
	int j, k, l, n, o;
	...
	if (readmem32b(data + start) == MAGIC_P41A)
		return 0;
