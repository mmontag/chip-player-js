...
TEST(test_openmpt_it_ins_flt_porta_reset)
{
	...
	f = fopen("openmpt/it/ins-flt-porta-reset.data", "r");

	opaque = xmp_create_context();
	xmp_load_module(opaque, "openmpt/it/ins-flt-porta-reset.it");
	...
	while (1) {
		...
		for (i = 0; i < m->mod.chn; i++) {
			...
			fail_unless(vi->filter.cutoff == cutoff, "cutoff mismatch");
		}
