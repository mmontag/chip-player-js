...
static int gdm_test(HANDLE *, char *, const int);
static int gdm_load (struct module_data *, HANDLE *, const int);
...
static int gdm_test(HANDLE *f, char *t, const int start)
{
	if (hread_32b(f) != MAGIC_GDM)
		return -1;

	hseek(f, start + 0x47, SEEK_SET);
	if (hread_32b(f) != MAGIC_GMFS)
		return -1;

	hseek(f, start + 4, SEEK_SET);
	read_title(f, t, 32);
...
static int gdm_load(struct module_data *m, HANDLE *f, const int start)
{
	...
	hseek(f, 32, SEEK_CUR);	/* skip author */
	...
	ins_ofs = hread_32l(f);
	smp_ofs = hread_32l(f);
	mod->ins = mod->smp = hread_8(f) + 1;
	mod->trk = mod->pat * mod->chn;
	
	MODULE_INFO();

	hseek(f, start + ord_ofs, SEEK_SET);
	...
	hseek(f, start + ins_ofs, SEEK_SET);
	...
	for (i = 0; i < mod->ins; i++) {
		...
		hseek(f, 12, SEEK_CUR);		/* skip filename */
		hread_8(f);			/* skip EMS handle */
	...
	hseek(f, start + pat_ofs, SEEK_SET);
	...
	hseek(f, start + smp_ofs, SEEK_SET);
