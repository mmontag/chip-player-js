...
TEST(test_api_set_mixer)
{
	...
	/* dsp */
	ret = xmp_set_mixer(opaque, XMP_MIXER_DSP, 255);
	fail_unless(ret == 0, "can't set XMP_MIXER_DSP");
	ret = xmp_get_mixer(opaque, XMP_MIXER_DSP);
	fail_unless(ret == 255, "can't get XMP_MIXER_DSP");

	/* mix */
	ret = xmp_set_mixer(opaque, XMP_MIXER_MIX, 0);
	fail_unless(ret == 0, "error setting mix");
	ret = xmp_get_mixer(opaque, XMP_MIXER_MIX);
	fail_unless(ret == 0, "can't get XMP_MIXER_MIX");

	ret = xmp_set_mixer(opaque, XMP_MIXER_MIX, 100);
	fail_unless(ret == 0, "error setting mix");
	ret = xmp_get_mixer(opaque, XMP_MIXER_MIX);
	...
	ret = xmp_set_mixer(opaque, XMP_MIXER_MIX, 50);
	fail_unless(ret == 0, "error setting mix");
	ret = xmp_get_mixer(opaque, XMP_MIXER_MIX);
	fail_unless(ret == 50, "can't get XMP_MIXER_MIX");

	ret = xmp_set_mixer(opaque, XMP_MIXER_MIX, 101);
	...
	/* amp */
	ret = xmp_set_mixer(opaque, XMP_MIXER_AMP, 0);
	fail_unless(ret == 0, "error setting amp");
	ret = xmp_get_mixer(opaque, XMP_MIXER_AMP);
	fail_unless(ret == 0, "can't get XMP_MIXER_AMP");

	ret = xmp_set_mixer(opaque, XMP_MIXER_AMP, 3);
	fail_unless(ret == 0, "error setting amp");
	ret = xmp_get_mixer(opaque, XMP_MIXER_AMP);
	fail_unless(ret == 3, "can't get XMP_MIXER_AMP");

	ret = xmp_set_mixer(opaque, XMP_MIXER_AMP, 2);
	fail_unless(ret == 0, "error setting amp");
	ret = xmp_get_mixer(opaque, XMP_MIXER_AMP);
	fail_unless(ret == 2, "can't get XMP_MIXER_AMP");

	ret = xmp_set_mixer(opaque, XMP_MIXER_AMP, 101);
	fail_unless(ret < 0, "error setting invalid amp");
	ret = xmp_set_mixer(opaque, XMP_MIXER_AMP, -1);
	fail_unless(ret < 0, "error setting invalid amp");
