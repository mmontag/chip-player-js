...
static int gal5_test(HANDLE *, char *, const int);
static int gal5_load(struct module_data *, HANDLE *, const int);
...
static int gal5_test(HANDLE *f, char *t, const int start)
{
	...
	if (hread_32b(f) != MAGIC4('A', 'M', ' ', ' '))
		return -1;

	if (hread_32b(f) != MAGIC4('I', 'N', 'I', 'T'))
		return -1;
...
static void get_init(struct module_data *m, int size, HANDLE *f, void *parm)
{
	...
	flags = hread_8(f);	/* bit 0: Amiga period */
	if (~flags & 0x01)
...
static void get_inst(struct module_data *m, int size, HANDLE *f, void *parm)
{
	...
	hseek(f, 290, SEEK_CUR);	/* Sample/note map, envelopes */
	mod->xxi[i].nsm = hread_16l(f);
	...
	hread_32b(f);	/* SAMP */
	hread_32b(f);	/* size */
	hread_32b(f);	/* unknown - usually 0x40000000 */

	hread(&mod->xxs[i].name, 1, 28, f);
	str_adj((char *)mod->xxs[i].name);

	hread_32b(f);	/* unknown - 0x0000 */
	hread_8(f);	/* unknown - 0x00 */

	mod->xxi[i].sub[0].sid = i;
	mod->xxi[i].vol = hread_8(f);
	mod->xxi[i].sub[0].pan = 0x80;
	mod->xxi[i].sub[0].vol = (hread_16l(f) + 1) / 512;
	flags = hread_16l(f);
	hread_16l(f);			/* unknown - 0x0080 */
...
static int gal5_load(struct module_data *m, HANDLE *f, const int start)
{
	struct xmp_module *mod = &m->mod;
	iff_handle handle;
	int i, offset;
